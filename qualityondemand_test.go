...
func TestQualityondemandGetQosProfileWithOptionalParams(t *testing.T) {
...
func TestQualityondemandGetQosProfilesWithOptionalParams(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Qualityondemand.GetQosProfiles(context.TODO(), camara.QualityondemandGetQosProfilesParams{
		Device: camara.QualityondemandGetQosProfilesParamsDevice{
			Ipv4Address: camara.QualityondemandGetQosProfilesParamsDeviceIpv4Address{
				PrivateAddress: camara.String("203.0.113.0"),
				PublicAddress:  camara.String("203.0.113.0"),
		...
		Name:        camara.String("voice"),
		Status:      camara.QosProfileStatusActive,
		XCorrelator: camara.String("b4333c46-49c0-4f62-80d7-f0ef930f1c46"),
	})
	if err != nil {
		var apierr *camara.Error
